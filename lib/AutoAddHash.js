...
module.exports = class {
	...
	addHash(sourceFile, targetFile, args, partialCode) {
		...
		urls.forEach(item => {
			try {
				if (this.cache[item]) {
					...
					let _md5 = compressor.compress(item);
		...
		if (md5s.length > 0) {
			let minFilePath = options.compressSuffix === false ? targetFile : targetFile.replace(/\.js$/i, `.${options.compressSuffix}.js`);
			args[1] = minFilePath + "?" + common.md5(md5s.join()).slice(0, this.hashLen);
		}
